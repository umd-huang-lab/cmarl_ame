...
# information sharing strageties
class ShareStrategy(Enum):
    ALWAYS = 0
    NO = 1
    NOISY = 2
...
class Agent(Entity):  # properties of agent entities
    def __init__(self):
        ...
        # other agents cannot see my position
        self.is_masked = False
        # information sharing strategy
        self.share_strategy = defaultdict(lambda: ShareStrategy.ALWAYS)
        # information sharing noise level
        self.share_noise = 0.5
...
class World(object):  # multi-agent world
    ...
    def step(self):
        ...
        # move landmark
        p_force = self.move_landmark(p_force)
    ...
    # gather agent action forces
    def move_landmark(self, p_force):
        num_agents = len(self.agents)
        # set applied forces
        for i, landmark in enumerate(self.landmarks):
            if landmark.movable:
                p_force[i+num_agents] = np.random.randn(*landmark.state.p_vel.shape)*2.0
        return p_force
